...
app.get(/^(?!\/auth|\/orders|\/services|\/categories|\/customers|\/tenants|\/messaging|\/users|\/faqs|\/faq-suggestions|\/delivery-zones|\/delivery-brackets|\/blocked-dates|\/promo-codes|\/public|\/webhook|\/conversations|\/push|\/finance).*/, (req, res) => {
...
app.listen(PORT, '0.0.0.0', async () => {
  ...
  try {
    ...
    const { rows: tenants } = await db.query(`SELECT id, name, fb_page_access_token, ig_user_id, custom_domain FROM tenants WHERE active=TRUE`);
    for (const t of tenants) {
      try {
        await setupMessengerProfile(t.fb_page_access_token, t.name, t.id, process.env.APP_URL, t.ig_user_id, t.custom_domain);
      } catch (e) {
