      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e8e8e0', borderRadius: 12, padding: '1.25rem' }}>
            <label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 5 }}>Send to</label>
            ...
            <label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 5 }}>Message</label>
            ...
            <button onClick={handleBlast} disabled={sending}
              style={{ width: '100%', padding: '9px', fontSize: 13, borderRadius: 6, cursor: 'pointer', background: sending ? '#aaa' : '#378ADD', color: '#fff', border: 'none', fontWeight: 500 }}>
        ...
        <div>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e8e8e0', borderRadius: 12, overflow: 'hidden' }}>
            {history.length === 0 ? (
              <div style={{ padding: '1.5rem', color: '#aaa', fontSize: 13, textAlign: 'center' }}>No blasts sent yet</div>
            ) : history.map(b => (
              <div key={b.id} style={{ padding: '10px 14px', borderTop: '0.5px solid #f0f0ec', fontSize: 13 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#888' }}>{new Date(b.created_at).toLocaleString()}</span>
                </div>
                <div style={{ fontSize: 12, color: '#888', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{b.message}</div>
              </div>
            ))}
          </div>
        </div>
