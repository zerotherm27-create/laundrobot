...
export default function Customers() {
  ...
  // `showCloseButton` is false in the sheet since swipe-down / the overlay
  // already close it.
  function renderDetail(showCloseButton) {
    return (
      <>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderTop: '0.5px solid #f0f0ec', fontSize: 13 }}>
          ...
          <button onClick={toggleReviewed} style={{
        ...
        <>
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[1, 2, 3, 4].map(i => <div key={i} className="skeleton" style={{ height: 58 }} />)}
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filtered.map(c => (
                <button key={c.id} onClick={() => setSelected(c)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderRadius: 10, border: '0.5px solid #e8e8e0', background: '#fff', textAlign: 'left', width: '100%' }}>
                  <Avatar name={c.name || '?'} size={36} bg="#EEEDFE" color="#534AB7" />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 500, fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name || 'Unknown'}</div>
                    <div style={{ fontSize: 12, color: '#374151' }}>{c.phone || '—'}</div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#3B6D11' }}>₱{Number(c.total_spent || 0).toLocaleString()}</div>
                    <div style={{ fontSize: 11, color: '#374151' }}>{c.total_orders || 0} orders</div>
                  </div>
                </button>
              ))}
              {filtered.length === 0 && (
                <div style={{ padding: '2rem', textAlign: 'center', color: '#374151', fontSize: 13 }}>No customers found</div>
              )}
            </div>
          )}

          {/* Reusing `.modal-overlay` (not just its look) is deliberate: the
              app's PullToRefresh guard checks for that class to skip a pull
              while any edit surface is open, so this sheet gets the same
              protection for free. */}
          <Drawer.Root open={!!selected} onOpenChange={open => !open && setSelected(null)}>
            <Drawer.Portal>
              <Drawer.Overlay className="modal-overlay" style={{ background: 'rgba(0,0,0,.45)' }} />
              <Drawer.Content style={{
                position: 'fixed', bottom: 0, left: 0, right: 0, maxHeight: '85vh', overflowY: 'auto',
                background: '#fff', borderTopLeftRadius: 16, borderTopRightRadius: 16,
                padding: '1.25rem', paddingBottom: 'calc(env(safe-area-inset-bottom) + 1.25rem)', zIndex: 201,
              }}>
                {selected && renderDetail(false)}
              </Drawer.Content>
            </Drawer.Portal>
          </Drawer.Root>
        </>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e8e8e0', borderRadius: 12, overflow: 'hidden' }}>
            {loading ? (
              <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[1, 2, 3, 4].map(i => <div key={i} className="skeleton" style={{ height: 20 }} />)}
              </div>
            ) : (
