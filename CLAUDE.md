...
- **How it actually works now (`backend/utils/botEchoTracker.js`) — metadata tag, NOT a counter:** every outbound message goes through `post()` in `utils/messenger.js` / `utils/instagram.js`, which stamps `message.metadata = BOT_METADATA_TAG` (`'laundrobot_ai_v1'`). Meta round-trips that string back in the echo at `message.metadata`; a human's inbox reply has none. The webhook calls `isBotOwnEcho(event.message, recipientId)`: if `message.metadata === BOT_METADATA_TAG` it's ours (no pause); otherwise a human typed it → `pauseAiForCustomer`. This is **stateless** — it survives process restarts and multiple replicas — and never touches `app_id`. Echo branches log `metadata` + `ownEcho` for debugging. Regression test: `backend/utils/botEchoTracker.test.js` (`node --test utils/botEchoTracker.test.js`).
- **Why the metadata tag replaced the old counter:** the previous mechanism counted bot sends in-memory (`noteBotSend`/`isOwnEcho`) and decremented on each echo. A count can't tell *which* message an echo belongs to, so Meta's out-of-order / dropped echoes let a human reply consume a still-outstanding bot send and get mislabeled as a bot echo → AI kept talking. That heuristic is now a **fallback only**, used when an echo carries NO `metadata` field at all (older/odd API versions, possibly some IG paths). The counter is still in-process memory (single-instance); the metadata path is what makes the fix replica-safe. **Do not make the counter the primary path again, and never add `app_id` discrimination.**
