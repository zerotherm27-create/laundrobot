...
- **How it actually works now (`backend/utils/botEchoTracker.js`):** every outbound message goes through `post()` in `utils/messenger.js` / `utils/instagram.js`, which calls `noteBotSend(recipientId)` (a per-recipient counter, NOT for `sender_action`/typing). Each incoming `is_echo` calls `isOwnEcho(recipientId)` — if there's an outstanding bot-send it's ours (consume one, no pause); otherwise a human typed it → `pauseAiForCustomer`. Independent of `app_id`. The echo branches still log the real `app_id` for debugging: `[webhook] echo … trackedOwnEcho: <bool>`.
- **Caveat: the counter is in-process memory → assumes a SINGLE backend instance** (current Railway setup). If you scale to multiple replicas, a send on one instance won't be seen by an echo on another (→ spurious pauses). Move the counter into the DB (e.g. a `pending_bot_echoes` column on `conversations`, keyed by `fb_user_id`) before scaling out.
